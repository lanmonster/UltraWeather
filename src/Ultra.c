...
const int WEEKDAY_RESOURCE_IDS[] = {
    ...
    RESOURCE_ID_IMAGE_MON,
    RESOURCE_ID_IMAGE_TUE,
    RESOURCE_ID_IMAGE_WED,
    RESOURCE_ID_IMAGE_THU,
    RESOURCE_ID_IMAGE_FRI,
    RESOURCE_ID_IMAGE_SAT
};

static void update(struct tm *current_time, BatteryChargeState charge_state)
{
    ...
    if( !bluetooth_connection_service_peek() )
    {
        ...
        bluetooth_image = gbitmap_create_with_resource(RESOURCE_ID_IMAGE_BLUETOOTH_DISCONNECTED);
    }
    else
    {
        gbitmap_destroy( bluetooth_image );
    ...
    if (charge_state.charge_percent <= 75 && charge_state.charge_percent > 50)
    {
        gbitmap_destroy( battery_image );
        battery_image = gbitmap_create_with_resource(BATTERY_LEVEL_RESOURCE_IDS[1]);
    }
    if (charge_state.charge_percent <= 50 && charge_state.charge_percent > 25)
    {
        gbitmap_destroy( battery_image );
        battery_image = gbitmap_create_with_resource(BATTERY_LEVEL_RESOURCE_IDS[2]);
    }
    if (charge_state.charge_percent <= 25 && charge_state.charge_percent > 0)
    {
        gbitmap_destroy( battery_image );
...
static void init(void)
{
    ...
    charging_layer = text_layer_create((GRect){ .origin = { 0, 10 }, .size = bounds.size });
    ...
    if( charge_state.is_charging )
    {
        text_layer_set_text( charging_layer, "charging" );
    }
    else
    {
        text_layer_set_text( charging_layer, "discharging" );
    }

    battery_layer = text_layer_create((GRect){ .origin = { 0, 25 }, .size = bounds.size });
    ...
    date_layer = text_layer_create((GRect){ .origin = { 0, 140 }, .size = bounds.size });
    ...
    dayOfWeek_image = gbitmap_create_with_resource(RESOURCE_ID_IMAGE_MON);
    dayOfWeek_layer = bitmap_layer_create((GRect){ .origin = { 0, 40 }, .size = bounds.size });
